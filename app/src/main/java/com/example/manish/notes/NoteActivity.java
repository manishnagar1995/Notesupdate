...
public class NoteActivity extends AppCompatActivity {
    ...
    private String mFileName,timefinder;
    private Note mLoadedNote = null;

    static int mYear,mMonth,mDay,mHour,mMinute,form;
    private EditText title,content;
    TextView datex,timex;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        title = (EditText) findViewById(R.id.titlex);
        ...
        if(mFileName != null && !mFileName.isEmpty() && mFileName.endsWith(Utilities.FILE_EXTENSION)) {
           mLoadedNote = Utilities.getNoteByFileName(getApplicationContext(), mFileName);
            if (mLoadedNote != null) {
                ...
                title.setText(mLoadedNote.getTitle());
                content.setText(mLoadedNote.getContent());
    ...
    private void updateDisplay() {
        ...
        timex.setText(new StringBuilder().append(mHour).append(":").append(mMinute).append(" ").append(timefinder));
    }
    ...
    /**
     * Record Function is added using the onActivityResult function which is invoked by spwwch reconizer
     */
    ...
    private void actionCancel() {
        ...
        if(!checkNoteAltred()) { //if note is not altered by user (user only viewed the note/or did not write anything)
            finish();
            //just exit the activity and go back to MainActivity
        }
     ...
     * true if note is changed, otherwise false
     */
    private boolean checkNoteAltred() {
        if(mIsViewingOrUpdating) { //if in view/update mode
            return mLoadedNote != null && (!title.getText().toString().equalsIgnoreCase(mLoadedNote.getTitle())
                    || !content.getText().toString().equalsIgnoreCase(mLoadedNote.getContent()));
        } else { //if in new note mode
            return !title.getText().toString().isEmpty() || !content.getText().toString().isEmpty();
        }
    }
    ...
    private void validateAndSaveNote() {
        ...
        if(Utilities.saveNote(this, new Note(mNoteCreationTime, titlex, contentx))) { //success!
